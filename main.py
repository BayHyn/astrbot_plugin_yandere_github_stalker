...
from astrbot.core.message.message_event_result import MessageChain, EventResultType
...
class YandereGithubStalker(Star):
        ...
        try:
            ...
            try:
                ...
                if image_path:
                    ...
                    return event.image_result(image_path).stop_event()
    ...
    async def add_user(self, event: AstrMessageEvent, username: str):
        ...
        try:
            ...
            try:
                ...
                return event.plain_result(f"❌ 验证用户失败: {e}").stop_event()
    ...
    async def enable_session(self, event: AstrMessageEvent):
        ...
        try:
            ...
            self.config.update({"target_sessions": target_sessions})  # 修复这里的参数传递
