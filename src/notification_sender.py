...
class NotificationSender:
    ...
    def _validate_session(self, session: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Yandere Github Stalker: 验证会话ID {session} 格式：{'合法' if is_valid else '不合法'}")
            return is_valid
    ...
    async def _send_notification(self, message_chain: MessageChain, target_sessions: List[str]) -> bool:
        ...
        """
        success = True
        logger.debug(f"Yandere Github Stalker: 准备发送通知到 {len(target_sessions)} 个会话")
        
        for session in target_sessions:
            try:
                if not self._validate_session(session):
                    logger.warning(f"Yandere Github Stalker: 跳过无效会话ID: {session}")
                    continue
        ...
        logger.debug(f"Yandere Github Stalker: 通知发送{'成功' if success else '失败'}")
        return success

    async def send_image_notification(self, username: str, event: Dict[str, Any], target_sessions: List[str]) -> bool:
        """
        发送图片通知
        :return: 是否发送成功
        """
        try:
            ...
            logger.debug(f"Yandere Github Stalker: 准备为用户 {username} 的事件 {event_id}（类型：{event_type}）生成图片通知")
            
            html_content = self.notification_renderer.render_html(username, event)
            ...
            if not img:
                logger.error(f"Yandere Github Stalker: 无法从路径 {image_path} 加载图片")
                return False
    ...
    async def send_text_notification(self, username: str, event: Dict[str, Any], target_sessions: List[str]) -> bool:
        """
        发送文本通知
        :return: 是否发送成功
        """
        try:
            event_id = event.get("id")
            event_type = event.get("type")
            logger.debug(f"Yandere Github Stalker: 准备为用户 {username} 的事件 {event_id}（类型：{event_type}）生成文本通知")
            
            text = self.notification_renderer.create_text_notification(username, event)
            ...
            if success:
                logger.debug(f"Yandere Github Stalker: 文本通知发送成功，事件ID：{event_id}")
            else:
                logger.warning(f"Yandere Github Stalker: 文本通知发送失败，事件ID：{event_id}")
            return success
        except Exception as e:
