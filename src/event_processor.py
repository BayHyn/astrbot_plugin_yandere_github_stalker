...
class EventProcessor:
    ...
    async def process_events(self, events: List[GitHubEventData]) -> List[GitHubEventData]:
        """处理事件列表，返回需要推送的新事件"""
        ...
        # 获取最后一次推送的事件时间
        last_pushed_time = await self.pushed_event_ids_manager.get_last_pushed_time()
        if last_pushed_time:
            logger.debug(f"Yandere Github Stalker: 上次推送时间：{last_pushed_time}")
        else:
            logger.debug("Yandere Github Stalker: 没有找到上次推送时间记录")
